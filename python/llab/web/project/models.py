...
class Project(models.Model):
    ...
    def post_receive_commit(self, old_rev, new_rev, refname):
        ...
        commit = Commit.create_from_sha(project, new_rev, branch)
        branch.update_commit_count()
...
class Branch(models.Model):
    ...
    # Statistics generated by the actual repository for this reference
    commit_count = models.IntegerField()

    def save(self, *args, **kwargs):
        if not self.pk and not self.commit_count:
            self.update_commit_count(save=False)
        return super(Branch, self).save(*args, **kwargs)

    def update_commit_count(self, save=True):
        self.commit_count = len(list(self.project.git.commits(self.name)))
        if save:
            self.save()
